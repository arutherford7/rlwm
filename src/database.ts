...
const FIREBASE_CONFIG: FirebaseOptions = (function() {
  if (config.nicks_db) {
    return {
...
let DB_META_DATA: DatabaseReference | null = null;
...
export function push_meta_data(data: TaskMetaData) {
  push_data(get_meta_data_db(), data);
}
...
export function init_db(on_success: () => void, on_err: (s: string) => void) {
  ...
  const set_db_refs = (uuid: string) => {
    uuid = config.is_debug_db_user ? 'debug-user' : `${uuid}-${random_alpha_numeric_string(4)}`;
